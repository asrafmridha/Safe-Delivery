...
@section('content')
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Objectives</h1>
        </div>
        <div class="col-sm-6">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Objective List </h3>
                        <a href="{{ route('admin.objective.create') }}" class="btn btn-success float-right">
                            <i class="fa fa-pencil-alt"></i> Add Objective
                        </a>
                    </div>
                    <div class="card-body">
  ...
  <script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.objective.getObjectives') !!}',
            columns: [
        ...
        $('#yajraDatatable').on('click', '.view-modal', function(){
            var objective_id = $(this).attr('objective_id');
            var url = "{{ route('admin.objective.show', ":objective_id") }}";
            url = url.replace(':objective_id', objective_id);
            $('#showResult').html('');
        ...
        $('#yajraDatatable').on('click', '.updateStatus', function(){
            ...
            var objective_id  = status_object.attr('objective_id');
            var status    = status_object.attr('status');
            var url       = "{{ route('admin.objective.updateStatus') }}";
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            var status_object = $(this);
            var objective_id   = status_object.attr('objective_id');
            var url         = "{{ route('admin.objective.delete') }}";

            var sttaus = confirm("Are you sure delete this objective?");
