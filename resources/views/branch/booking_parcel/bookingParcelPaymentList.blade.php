...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Parcel Payment List</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('branch.home') }}">Home</a></li>
                        <li class="breadcrumb-item active">Parcel Payments List</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"> Parcel Payment List </h3>
    ...
    <script>
        window.onload = function(){
            ...
            function load_data(status = '', from_date = '', to_date = ''){
                var table = $('#parcelPaymentTable').DataTable({
                    language : {
                    ...
                    serverSide: true,
                    ajax: {
                        url:'{!! route('branch.bookingParcelPayment.getBookingParcelPaymentList') !!}',
                        data:{
                            status      : status,
                            from_date   : from_date,
                            to_date     : to_date,
            ...
            $(document).on('click', '#printBtn', function(){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{!! route('branch.bookingParcelPayment.printBookingParcelPaymentList') !!}',
                    data: {
                        status   : status,
                        from_date   : from_date,
                        to_date     : to_date,
            ...
            $('#filter').click(function(){
                ...
                $('#parcelPaymentTable').DataTable().destroy();
            ...
            $('#parcelPaymentTable').on('click', '.view-modal', function(){
                var parcel_payment_id = $(this).attr('parcel_payment_id');
                var url = "{{ route('branch.bookingParcelPayment.viewBookingParcelPayment', ":parcel_payment_id") }}";
            ...
            $('#parcelPaymentTable').on('click', '.run-start-btn', function(){
            ...
            $('#parcelPaymentTable').on('click', '.run-cancel-btn', function(){
                var status_object = $(this);
                var rider_run_id   = status_object.attr('rider_run_id');
            ...
            $('#parcelPaymentTable').on('click', '.rider-run-reconciliation', function(){
