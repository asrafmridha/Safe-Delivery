                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            @if(check_permission('transaction list'))
                                <a href="{{route('transaction')}}" class="btn btn-primary ml-auto">
                                    Back
                                </a>
                            @endif
                        </ol>
                    ...
                    <table class="table  table-striped table-bordered" {{--style="font-size: 13px;"--}}>
                        ...
                        @foreach($items as $key=>$item)
                            ...
                            <tr>
                                ...
                                <td class="text-capitalize">{{$item->transaction_for}}</td>
...
{{--                                <td>{{$item->bdt_amount}}</td>--}}
                                <td>{{number_format($item->transaction_type=="credit"?$item->bdt_amount:0,2)}}</td>
                                <td>{{number_format($item->transaction_type=="debit"?$item->bdt_amount:0,2)}}</td>
                                <td>{{number_format($totalCredit - $totalDebit,2)}}</td>
                            </tr>

                        @endforeach
                        <tr>
                            <th colspan="13" class="text-right">Totals:</th>
                            <th>{{number_format($totalCredit,2)}}</th>
                            <th>{{number_format($totalDebit,2)}}</th>
                            <th>{{number_format($totalCredit - $totalDebit,2)}}</th>
                        </tr>
                        <tr>
                            <th rowspan="5" colspan="11" class="text-center">
                                <h2 class="mt-5">Summary</h2>
                            </th>
                        </tr>
                        <tr>
                            <th colspan="4" class="text-right">Total Transaction BDT Amount:</th>
                            <th>{{number_format($totalBdtAmount,2)}}</th>
                        </tr>
                        <tr>
    ...
    <script type="text/javascript">
        ...
        window.onload = function () {
            $(document).on('click', '#printBtn', function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{!! route('transaction.filter.print') !!}',
                    data: {
                        status: status,
