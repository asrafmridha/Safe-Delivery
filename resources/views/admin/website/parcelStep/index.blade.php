...
@section('content')
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Parcel Steps</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">Home</a></li>
            <li class="breadcrumb-item active">Parcel Steps</li>
          </ol>
        </div>
      </div>
    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Parcel Steps List </h3>
                        <a href="{{ route('admin.parcelStep.create') }}" class="btn btn-success float-right">
                            <i class="fa fa-pencil-alt"></i> Add Parcel Step
                        </a>
                    </div>
                    <div class="card-body">
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header bg-primary">
                      <h4 class="modal-title">View Parcel Step </h4>
  ...
  <script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.parcelStep.getParcelSteps') !!}',
            columns: [
        ...
        $('#yajraDatatable').on('click', '.view-modal', function(){
            var parcel_step_id = $(this).attr('parcel_step_id');
            var url = "{{ route('admin.parcelStep.show', ":parcel_step_id") }}";
            url = url.replace(':parcel_step_id', parcel_step_id);
            $('#showResult').html('');
            if(parcel_step_id.length != 0){
            $.ajax({
                cache   : false,
                type    : "GET",
        ...
        $('#yajraDatatable').on('click', '.updateStatus', function(){
            ...
            var parcel_step_id  = status_object.attr('parcel_step_id');
            var status    = status_object.attr('status');
            var url       = "{{ route('admin.parcelStep.updateStatus') }}";
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            var status_object = $(this);
            var parcel_step_id   = status_object.attr('parcel_step_id');
            var url         = "{{ route('admin.parcelStep.delete') }}";

            var sttaus = confirm("Are you sure delete this parcel step?");
