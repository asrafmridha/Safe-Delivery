...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['merchant_id'] = $merchant_id = auth()->guard('merchant')->user()->id;
        ...
        $return_branch = '';
        $branch_condition = '';
        if ($merchant_id != 0 && $merchant_id != '0') {
...
//        $data['total_parcel'] = Parcel::whereBetween('date', [$from_date, $to_date])->whereRaw('1' . $condition . $branch_condition)->count();
        ...
        // $data['total_delivery_assigned_parcel_charge'] = Parcel::whereBetween('parcel_date', [$from_date, $to_date])
        //     ->whereRaw('status >= 11 ' . $condition . $delivery_branch)->sum('delivery_charge');
        ...
        // $data['total_parcel_pickup_charge'] = Parcel::whereBetween('pickup_branch_date', [$from_date, $to_date])
        //     ->whereRaw('status >= ? ' . $condition . $pickup_branch, [11])->sum('delivery_charge');
        ...
        return view('merchant.report', $data);
    }
}
