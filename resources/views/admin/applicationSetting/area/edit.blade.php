                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name" id="name"
                                                       value="{{ $area->name ?? old('name') }}" class="form-control"
                                                       placeholder="Area Name" required>
                                            </div>
                                            <div class="form-group">
                                                <label for="post_code">Post Code</label>
                                                <input type="text" name="post_code" id="post_code"
                                                       value="{{ $area->post_code ??  old('post_code') }}"
                                                       class="form-control" placeholder="Area Post Code">
                                            </div>
                                            <div class="row">
                                                <div class="col-md-4 form-group">
                                                    <label for="city_id">Pathao City</label>
                                                    <select name="city_id" id="city_id" class="form-control select2"
                                                            style="width: 100%">
                                                        <option value="">Select City</option>
                                                        @foreach ($pathao_cities as $pathao_city)
                                                            <option
                                                                value="{{ $pathao_city['city_id'] }}" {{$area->pathao_city_id==$pathao_city['city_id']?'selected':''}}> {{ $pathao_city['city_name'] }} </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-4 form-group">
                                                    <label for="zone_id">Pathao Zone</label>
                                                    <select name="zone_id" id="zone_id" class="form-control select2"
                                                            style="width: 100%">
                                                        <option value="">Select Zone</option>
                                                        @if(isset($pathao_zones))
                                                            @foreach ($pathao_zones as $pathao_zone)
                                                                <option
                                                                    value="{{ $pathao_zone['zone_id'] }}" {{$area->pathao_zone_id==$pathao_zone['zone_id']?'selected':''}}> {{ $pathao_zone['zone_name'] }} </option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                                <div class="col-md-4 form-group">
                                                    <label for="area_id">Pathao Area</label>
                                                    <select name="area_id" id="area_id" class="form-control select2"
                                                            style="width: 100%">
                                                        <option value="">Select Area</option>
                                                        @if(isset($pathao_areas))
                                                            @foreach ($pathao_areas as $pathao_area)
                                                                <option
                                                                    value="{{ $pathao_area['area_id'] }}" {{$area->pathao_area_id==$pathao_area['area_id']?'selected':''}}> {{ $pathao_area['area_name'] }} </option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
...
@push('script_js')
    ...
    <script>
        $("#district_id").val('{{ $area->district_id }}');
        $("#upazila_id").val('{{ $area->upazila_id }}');
