...
<div class="content-header">
    ...
    <div class="row mb-2">
        <div class="col-sm-6">
        <h1 class="m-0 text-dark">Generate Return Rider Run </h1>
        </div>
        <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('branch.home') }}">Home</a></li>
            <li class="breadcrumb-item active">Generate Return Rider Run </li>
        </ol>
        </div>
    </div>
    </div>
  ...
  <div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form role="form" action="{{ route('branch.parcel.confirmReturnRiderRunGenerate') }}" method="POST" enctype="multipart/form-data" onsubmit="return createForm()">
                    @csrf
                    <input type="hidden" name="total_run_parcel" id="total_run_parcel" value="0" >
                    <div class="row">
                        <div class="col-md-12">
                            <fieldset>
                                <legend>Return Run </legend>
                                <div class="row">
                                    <div class="col-md-6">
                                        <table class="table ">
                                            <tr>
                                            ...
                                            <tr>
                                                <td colspan="3">
                                                    <textarea name="note" id="note" class="form-control" placeholder="Return Rider Run Note "></textarea>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                ...
                <div class="row">
                    <div class="col-md-12 row" style="margin-top: 20px;">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <input type="text" name="merchant_order_id" id="merchant_order_id" class="form-control" placeholder="Enter C. Number Or Merchant Order ID"
                                       style="font-size: 20px; box-shadow: 0 0 5px rgb(62, 196, 118);
                                    padding: 3px 0px 3px 3px;
                                    margin: 5px 1px 3px 0px;
                                    border: 1px solid rgb(62, 196, 118);">
                        ...
                        <div class="col-md-3">
                            <div class="form-group merchantSelect">
                                ...
                                <select name="merchant_id" id="merchant_id" class="form-control select2" style="font-size: 20px; box-shadow: 0 0 5px rgb(62, 196, 118);
                                    ...
                                    {{--@endforeach--}}
                                    @foreach ($merchants as $merchant)
                                        <option value="{{ $merchant->merchant->id }}" >
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped "  style="background-color:white;width: 100%">
                                ...
                                <tr  style="background-color: #a2bbca !important; font-family: Arial Black;font-size: 14px">
                                    ...
                                    <th width="20%" class="text-center">Return Reason </th>
                                    <th width="15%" class="text-center">Customer Name</th>
                                     <th width="15%" class="text-center">Collected Amount</th>
  ...
  <script>
    ...
    window.onload = function(){
        ...
        $('#addParcelInvoice').on('click', function(){
            ...
            $.ajax({
                ...
                url       : "{{ route('branch.parcel.returnRiderRunParcelAddCart') }}",
    ...
    function delete_parcel(itemId){
        $.ajax({
            ...
            url      : "{{ route('branch.parcel.returnRiderRunParcelDeleteCart') }}",
            error   : function(xhr){ alert("An error occurred: " + xhr.status + " " + xhr.statusText); },
            success : function (response){
                $('#show_rider_run_parcel').html(response);
            }
    ...
    function parcelResult(){
        ...
        $.ajax({
            ...
            url       : "{{ route('branch.parcel.returnReturnRiderRunParcel') }}",
            error     : function(xhr){ alert("An error occurred: " + xhr.status + " " + xhr.statusText); },
            success   : function(response){
    ...
    function createForm(){
        ...
        if(total_run_parcel == 0){
            toastr.error("Please Enter Return Run Parcel..");
            return false;
        }
