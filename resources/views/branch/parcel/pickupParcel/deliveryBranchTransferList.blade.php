    ...
    window.onload = function(){
        var table = $('#yajraDatatable').DataTable({
            ...
            columns: [
                ...
                { data: 'to_branch.name', name: 'to_branch.name' , class : "text-center", render: (data, type, row) =>  (row && row.to_branch && row.to_branch.name) ? data : "--" },
                { data: 'to_branch.address', name: 'to_branch.address' , class : "text-center",  render: (data, type, row) =>  (row && row.to_branch&& row.to_branch.address) ? data : "--"},
                { data: 'to_branch.contact_number', name: 'to_branch.contact_number' , class : "text-center",render: (data, type, row) =>  (row && row.to_branch && row.to_branch.contact_number) ? data : "--"},
        ...
        $(document).on('click', '#printBtn', function(){
            $.ajax({
                type: 'GET',
                url: '{!! route('branch.parcel.printDeliveryBranchTransferList') !!}',
                data: {},
                dataType: 'html',
        ...
        $(document).on('click', '.print-modal', function(){
            ...
            var url = "{{ route('branch.parcel.printDeliveryBranchTransfer', ":delivery_branch_transfer_id") }}";
