...
@section('content')
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Return Parcel List</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('rider.home') }}">Home</a></li>
            <li class="breadcrumb-item active">Return Parcels List</li>
          </ol>
        </div>
      </div>
    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Return Parcels List </h3>
                    </div>
                    <div class="card-body">
...
<script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('rider.parcel.getReturnParcelList') !!}',
            columns: [
        ...
        $('#yajraDatatable').on('click', '.return-request-accept-btn', function(){
            var status_object = $(this);
            var parcel_id   = status_object.attr('parcel_id');
            var url         = "{{ route('rider.parcel.parcelReturnRequestAccept') }}";
        ...
        $('#yajraDatatable').on('click', '.return-request-reject-btn', function(){
            var status_object = $(this);
            var parcel_id   = status_object.attr('parcel_id');
            var url         = "{{ route('rider.parcel.parcelReturnRequestReject') }}";
        ...
        $('#yajraDatatable').on('click', '.return-complete-btn', function(){
            var parcel_id = $(this).attr('parcel_id');
            var url = "{{ route('rider.parcel.parcelReturnComplete', ":parcel_id") }}";
            url = url.replace(':parcel_id', parcel_id);
            $('#showResult').html('');
            if(parcel_id.length != 0){
                $.ajax({
