    ...
    <div class="login-box">
        ...
        <div class="card">
            <div class="card-body login-card-body">
                ...
                <br>
                <p class="login-box-msg text-success" style="font-size: 22px; font-weight: bold">Confirm Merchant Rest Password </p>
                @include('layouts.merchant_layout.merchant_session_alert')

                @if($checkTime <= 60 && $verification == 0)
                    <form action="{{ route('merchant.resetPassword') }}" method="post">
                        @csrf
                        <input type="hidden" name="token" value="{{ $token }}">

                        <div class="input-group mb-3">
                            <input type="email" name="email" class="form-control" placeholder="Email Address" value="{{ $merchant->email }}" readonly >
