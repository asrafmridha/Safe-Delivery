...
class BookingParcelAssignRejectController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.booking_parcel.branchVehicleAssignList', $data);
    }
...
//        dd($data['bookingParcel']);
    ...
    public function assignVehicleToWarehouse(){
        $data               = [];
        $data['main_menu']  = 'booking';
    ...
    public function getParcelListForVehicleToWareHouseAssign(Request $request){
        ...
        if($request->booking_branch_id != 'all'){
            $bookingParcel = BookingParcel::with(['receiver_branch'])
                                    ...
                                    ->whereNotIn('vehicle_warehouse_status', array(0,2))
                                    ->get();
        }else{
            $bookingParcel = BookingParcel::with(['receiver_branch'])
                                    ->where('vehicle_id', $request->vehicle_id)
...
//            return $ex->getMessage();
    ...
    public function bookingParcelOperation(){
        ...
        $data['receiver_branches']     = BookingParcel::with(array('receiver_branch'=>function($query){
        ...
        return view('admin.booking_parcel.bookingParcelOperation', $data);
    }
    ...
    public function getParcelListForVehicleToWarehouseReceive(Request $request){
        ...
        if($request->vehicle_id != 'all'){
            $bookingParcel = BookingParcel::with(['receiver_branch'])
                ->where('receiver_warehouse_id', $request->warehouse_id)
                ->where('vehicle_id', $request->vehicle_id)
        ...
        if(count($bookingParcel) > 0) {
            ...
            $value_data = '<tr>
                               <td colspan="7" style="text-align: center;">Parcel Not Found!</td>
                            </tr>';
            return response()->json($value_data);
        }
        //return view('admin.booking_parcel.branchVehicleAssignParcelList', $data);
...
//            dd($request->all());
            ...
            $data_parcel_update    = [
                'status'                    => 4,
                'vehicle_id'                => 0,
                'sender_warehouse_id'       => $request->get('receive_warehouse_id'),
                'sender_warehouse_type'     => $request->get('receive_warehouse_type'),
            ...
            if($request->total_assign_parcel > 0) {
                ...
                foreach ($request->get('receive_parcel_id') as $k=>$v_id) {
                    ...
                    $data_parcel_logs[] = [
                        ...
                        'receiver_warehouse_id'         => $request->get('receive_warehouse_id'),
                        'receiver_warehouse_type'       => $request->get('receive_warehouse_type'),
                        'note'                          => $request->get('note'),
                        'status'                        => 4,
                        'vehicle_warehouse_status'      => 2,
                        'created_admin_user_id'         => $user_id,
...
//            return $ex->getMessage();
    ...
    public function getParcelListForWarehouseToVehicleWarehouseAssign(Request $request){
        ...
        if($request->warehouse_id != '0' && $request->receiver_branch_id != "") {
            $bookingParcel = BookingParcel::with(['receiver_branch'])
                ->where('sender_warehouse_id', $request->warehouse_id)
                ...
                foreach ($request->get('assign_parcel_id') as $k=>$v_id) {
                    ...
                    if($request->get('destination_branch') == 'yes') {
                        $data_parcel_logs[] = [
                            'booking_id'                    => $booking_id,
                            'vehicle_id'                    => $request->get('assign_vehicle_id'),
                            'sender_warehouse_id'           => $request->get('from_warehouse_id'),
                            'sender_warehouse_type'         => $request->get('from_warehouse_type'),
                            'vehicle_warehouse_status'      => 1,
                            'note'                          => $request->get('note'),
                            'status'                        => 6,
                            'created_admin_user_id'         => $user_id,
