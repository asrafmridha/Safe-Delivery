...
<div class="content-header">
    ...
    <div class="row mb-2">
        <div class="col-sm-6">
        <h1 class="m-0 text-dark">Generate Return Branch Transfer (Trip) </h1>
        </div>
        <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('branch.home') }}">Home</a></li>
            <li class="breadcrumb-item active">Generate Return Branch Transfer (Trip) </li>
        </ol>
        </div>
    </div>
    </div>
  ...
  <div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form role="form" action="{{ route('branch.parcel.confirmReturnBranchTransferGenerateEdit', $returnBranchTransfer->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return createForm()">
                    @method('patch')
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <fieldset>
                                <legend>Return Branch Transfer</legend>
                                <div class="row">
                                    <div class="col-md-6">
                                        <table class="table ">
                                            <tr>
                                            ...
                                            <tr>
                                                <th >Date</th>
                                                <td colspan="2">
                                                    <input type="date" name="date" id="date" value="{{ date('Y-m-d', strtotime($returnBranchTransfer->create_date_time)) }}" class="form-control " required>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td style="width: 5%"> : </td>
                                                <td style="width: 55%">
                                                    <span id="view_branch_name">{{ $returnBranchTransfer->to_branch->name }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td style="width: 5%"> : </td>
                                                <td style="width: 55%">
                                                    <span id="view_branch_contact_number">{{ $returnBranchTransfer->to_branch->contact_number }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td style="width: 5%"> : </td>
                                                <td style="width: 55%">
                                                    <span id="view_branch_address">{{ $returnBranchTransfer->to_branch->address }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td style="width: 5%"> : </td>
                                                <td style="width: 55%">
                                                    <span id="view_total_transfer_parcel"> {{ $returnBranchTransfer->total_transfer_parcel }} </span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="3">
                                                    <textarea name="note" id="note" class="form-control" placeholder="Return Branch Transfer (Trip) Note "></textarea>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                    <div class="col-md-6">
                                        <fieldset id="div_return_branch_transfer_parcel">
                                            <legend>Return Branch Transfer Parcel </legend>
                                            <div class="row">
                                                <div class="col-sm-12" id="show_return_branch_transfer_parcel">
...
<script>
    $("#branch_id").val({{ $returnBranchTransfer->to_branch_id }});
    ...
    window.onload = function(){
        ...
        $('#addParcelInvoice').on('click', function(){
            ...
            $.ajax({
                ...
                url       : "{{ route('branch.parcel.returnBranchTransferParcelAddCart') }}",
                error     : function(xhr){ alert("An error occurred: " + xhr.status + " " + xhr.statusText); },
                success   : function(response){
                    $("#show_return_branch_transfer_parcel").html(response);
    ...
    function delete_parcel(itemId){
        $.ajax({
            ...
            url      : "{{ route('branch.parcel.returnBranchTransferParcelDeleteCart') }}",
            error   : function(xhr){ alert("An error occurred: " + xhr.status + " " + xhr.statusText); },
            success : function (response){
                $('#show_return_branch_transfer_parcel').html(response);
            }
    ...
    function parcelResult(){
        ...
        $.ajax({
            ...
            url       : "{{ route('branch.parcel.returnReturnBranchTransferParcel') }}",
            error     : function(xhr){ alert("An error occurred: " + xhr.status + " " + xhr.statusText); },
            success   : function(response){
    ...
    function createForm(){
        ...
        if(total_transfer_parcel == 0){
            toastr.error("Please Enter Return Branch Transfer Parcel..");
            return false;
        }
    }
