<button class="dropdown-toggle notification" type="button" data-toggle="dropdown" aria-haspopup="true"
        aria-expanded="false">
    Pending <i class="far fa-bell"></i><sup class="bg-danger px-1  rounded-circle">{{count($orders)}}</sup>
</button>
<div class="dropdown-menu dropdown-menu-right dropdown-menu-xl">
    <div class="dropdown_heading">
        <p>You have <strong class="text-primary">{{count($orders)}}</strong> pending.</p>
        <!-- list group -->
        ...
        @endforeach
        <!-- view all -->
        <a href="{{route('transaction.pending')}}" class="view_all">view all</a>
    </div>
</div>
