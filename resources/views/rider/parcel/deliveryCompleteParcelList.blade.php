...
@section('content')
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Delivery Complete Parcel List</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('rider.home') }}">Home</a></li>
            <li class="breadcrumb-item active">Delivery Complete Parcels List</li>
          </ol>
        </div>
      </div>
    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Delivery Complete Parcels List </h3>
                    </div>
                    <div class="card-body">
                        <table id="yajraDatatable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th width="8%" class="text-center"> Collected Amount</th>
...
<script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('rider.parcel.getDeliveryCompleteParcelList') !!}',
            columns: [
...
//                { data: 'action', name: 'action', orderable: false , searchable: false , class : "text-center"}
        ...
        $('#yajraDatatable').on('click', '.delivery-request-accept-btn', function(){
            var status_object = $(this);
            var parcel_id   = status_object.attr('parcel_id');
            var url         = "{{ route('rider.parcel.parcelDeliveryRequestAccept') }}";
        ...
        $('#yajraDatatable').on('click', '.delivery-request-reject-btn', function(){
            var status_object = $(this);
            var parcel_id   = status_object.attr('parcel_id');
            var url         = "{{ route('rider.parcel.parcelDeliveryRequestReject') }}";
        ...
        $('#yajraDatatable').on('click', '.delivery-complete-btn', function(){
            var parcel_id = $(this).attr('parcel_id');
            var url = "{{ route('rider.parcel.parcelDeliveryComplete', ":parcel_id") }}";
            url = url.replace(':parcel_id', parcel_id);
            $('#showResult').html('');
            if(parcel_id.length != 0){
                $.ajax({
        ...
        $('#yajraDatatable').on('click', '.processParcel', function(){
            var parcel_id = $(this).attr('parcel_id');
            var url = "{{ route('rider.parcel.processParcel', ":parcel_id") }}";
            url = url.replace(':parcel_id', parcel_id);
            $('#showResult').html('');
            if(parcel_id.length != 0){
                $.ajax({
