      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Rider Payments</li>
          </ol>
        </div>
      </div>
    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Rider Payment List </h3>
                        <a href="{{ route('admin.rider.payment.create') }}" class="btn btn-success float-right">
                            ...
                            <table id="yajraDatatable" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center"> KM Commission</th>
                                        <th class="text-center"> Parcel Commission</th>
                                        <th class="text-center"> Weight Commission</th>
...
@push('script_js')
  <script>
    window.onload = function(){
        ...
        function load_data(branch_id ='', rider_id ='', from_date ='', to_date ='') {
            var table = $('#yajraDatatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{!! route('admin.rider.payment.datatable') !!}',
                    data: {
                        branch_id   :branch_id,
                        rider_id    :rider_id,
                        from_date   :from_date,
                ...
                columns: [
                    ...
                    { data: 'rider.branch.name', name: 'rider.branch.name' , class : "text-center"},
                    { data: 'payment_month', name: 'payment_month' , class : "text-center"},
                    { data: 'salary_amount', name: 'salary_amount' , class : "text-center"},
                    { data: 'total_km_commission', name: 'total_km_commission' , class : "text-center"},
                    { data: 'total_parcel_commission', name: 'total_parcel_commission' , class : "text-center"},
                    { data: 'total_weight_commission', name: 'total_weight_commission' , class : "text-center"},
...
//                { data: 'action', name: 'action', orderable: false , searchable: false , class : "text-center"}
        ...
        $('#filter').click(function(){
            ...
            load_data(branch_id, rider_id, from_date, to_date);
        });

        $('#yajraDatatable').on('click', '.view-modal', function(){
            var rider_id = $(this).attr('rider_id');
            var url = "{{ route('admin.staff.show', ":rider_id") }}";
            url = url.replace(':rider_id', rider_id);
            $('#showResult').html('');
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            ...
            var rider_id   = status_object.attr('rider_id');
