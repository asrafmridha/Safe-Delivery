...
//                                ->get();
        ...
        if($request->get('filter_type') == 1) {
            ...
            $sql = "select m.id, m.m_id, m.company_name, m.address, m.contact_number, p.date, count(p.id) as count_parcel from merchants m join parcels p on p.merchant_id = m.id where m.status = 1 and p.pickup_branch_id != '' and p.status = 1 and p.date = '{$current_date}' and p.deleted_at IS NULL  group by p.date, m.id, m.m_id, m.company_name, m.address, m.contact_number";
            $merchant_with_parcel = DB::select(DB::raw($sql));

        }elseif($request->get('filter_type') == 2){

            $sql = "select m.id, m.m_id, m.company_name, m.address, m.contact_number, count(p.id) as count_parcel from merchants m join parcels p on p.merchant_id = m.id where m.status = 1 and p.pickup_branch_id != '' and p.status in (11,13,15) and p.date = '{$current_date}' and p.deleted_at IS NULL group by m.id, m.m_id, m.company_name, m.address, m.contact_number";

            $merchant_with_parcel = DB::select(DB::raw($sql));

        }elseif($request->get('filter_type') == 3){

            $sql = "select m.id, m.m_id, m.company_name, m.address, m.contact_number, count(p.id) as count_parcel from merchants m join parcels p on p.merchant_id = m.id where m.status = 1 and p.pickup_branch_id != '' and p.status >= 11 and p.deleted_at IS NULL group by m.id, m.m_id, m.company_name, m.address, m.contact_number";

            $merchant_with_parcel = DB::select(DB::raw($sql));

        }elseif($request->get('filter_type') == 4) {

            $sql = "select m.id, m.m_id, m.company_name, m.address, m.contact_number, p.date, count(p.id) as count_parcel from merchants m join parcels p on p.merchant_id = m.id where m.status = 1 and p.pickup_branch_id != '' and p.status = 1 and p.deleted_at IS NULL  group by p.date, m.id, m.m_id, m.company_name, m.address, m.contact_number";
            $merchant_with_parcel = DB::select(DB::raw($sql));
        }else{
...
//dd($data);
        return view('admin.parcel.parcelFilterList', $data);
    }
