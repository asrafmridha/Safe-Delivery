            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Riders List </h3>
                        @if(auth()->guard('admin')->user()->type == 1 || auth()->guard('admin')->user()->type == 3)
                            <a href="{{ route('admin.rider.create') }}" class="btn btn-success float-right">
                                <i class="fa fa-pencil-alt"></i> Add Rider
                            </a>
                        @endif
  ...
  <script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.rider.getRiders') !!}',
            columns: [
                { data: 'DT_RowIndex', orderable: false , searchable: false, class : "text-center"},
                { data: 'r_id', name: 'r_id' , class : "text-center"},
                ...
                { data: 'salary', name: 'salary' , searchable: false , class : "text-center"},
        ...
        $('#yajraDatatable').on('click', '.view-modal', function(){
            ...
            var url = "{{ route('admin.rider.show', ":rider_id") }}";
        ...
        $('#yajraDatatable').on('click', '.updateStatus', function(){
            ...
            var url             = "{{ route('admin.rider.updateStatus') }}";
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            var status_object = $(this);
            var rider_id   = status_object.attr('rider_id');
            var url         = "{{ route('admin.rider.delete') }}";

            var sttaus = confirm("Are you sure delete this rider?");
        ...
        $(document).on('click', '#printBtn', function(){
            $.ajax({
                type: 'GET',
                url: '{!! route('admin.rider.printRiders') !!}',
                data: {},
                dataType: 'html',
